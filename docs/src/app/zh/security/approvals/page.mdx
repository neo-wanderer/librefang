...
trusted_senders = []                   # 对低风险工具直接放行的用户 ID 列表（高风险工具仍需审批）
...
3. `trusted_senders` 只授予一方自动化 ID，切勿授予公共 webhook ID。
   受信任发送者只对低风险工具免审批。
   高风险工具——`shell_exec`、`file_write`、`file_delete`、`apply_patch`，以及控制面工具 `agent_spawn` / `agent_kill` / `config_set` / `kernel_reload`——始终需要显式审批，并仍受 channel deny 规则约束，即便来自受信任发送者。
   该列表是便利性豁免，而非对代码执行或破坏性变更的全面放行。
