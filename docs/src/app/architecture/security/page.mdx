...
Variables not in these lists and not in one of the allowlists are **never** passed to the child process.
The two allowlists carry different trust levels (#6458): the operator's own `exec_policy.allowed_env_vars` may carry secret-shaped names (`MYTOOL_KEYRING_PASSWORD`, `FOO_API_TOKEN`, ...) — only the daemon's own reserved secrets (`LIBREFANG_VAULT_KEY`, provider API keys) are refused unconditionally.
Untrusted passthrough lists (a marketplace hand's `[[requires]]` / settings names) are additionally filtered through the credential-word heuristic, at assembly time and again at spawn time.
Refused names are returned to the caller and surfaced in the tool output.
